        ...
        public ActionResult IndexPost(PageModel model)
        {
            ...
            else if (result > 0)
            {
                var getModuleID = from m in db.AslMenumstDbSet
                  ...
                  // (db.AslMenumstDbSet.Where(
                   //    d => d.MODULENM == model.aslMenumst.MODULENM).Select(d=>d.MODULEID)).Distinct().ToList();
                ...
                if (model.aslMenumst.MODULENM != null && model.aslMenu.MENUTP != null)
                {
                    TempData["ASLMenuMst_MODULEID"] = model.aslMenumst.MODULEID;
