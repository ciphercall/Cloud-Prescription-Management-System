...
PrescribeApp.controller("ApiPrescribeController", function ($scope, $http) {
    ...
    // Get all 1st, 2nd, 3rd, 4th,5th Data (Update Page)
    $scope.search = function (event) {
        ...
        if (TRANSDT != "" && TRANSNO != "" && RXPNM != "" && RXPID != "" && RXPIDM!="") {

            var compid = $('#txtcompid').val();
            var transno = $('#txt_TransNo').val();
            

            //Grid view load (1st part)
        ...
        else if (TRANSDT == "" && TRANSNO == "") {
            $('#txt_TransDate').val("");
            $('#txt_TransNo').val("");
            $('#txt_RxpPIDM').val("");
            $('#txt_PatientName').val("");
            $('#Hiddentxt_RxPID').val("");
            alert("Please input TransDate and Memo No field !!");
        }
        else {
